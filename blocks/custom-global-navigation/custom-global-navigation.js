...
const plainHTMLPromise = (async () => {
  ...
  const mepGnav = getConfig()?.mep?.inBlock?.['custom-global-navigation'];
...
class Gnav {
  ...
  decorateMainNavItem = (item, index) => {
    ...
    const makeTabActive = (popup) => {
      ...
      const tabbuttons = popup.querySelectorAll('.custom-global-navigation .tabs button');
      const tabpanels = popup.querySelectorAll('.custom-global-navigation .tab-content [role="tabpanel"]');
